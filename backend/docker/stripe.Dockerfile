...
RUN test -n "${BWS_PROJECT_NAME_FOR_STRIPE_WEBHOOK_CHECK}" || ( \
  echo "Error: BWS_PROJECT_NAME_FOR_STRIPE_WEBHOOK_CHECK is not set! Use --build-arg BWS_PROJECT_NAME_FOR_STRIPE_WEBHOOK_CHECK=xxx" && \
  exit 1 \
);
...
ARG BWS_PROJECT_NAME_FOR_STRIPE_LISTENER
...
RUN test -n "${BWS_PROJECT_NAME_FOR_STRIPE_LISTENER}" || ( \
  echo "Error: BWS_PROJECT_NAME_FOR_STRIPE_LISTENER is not set! Use --build-arg BWS_PROJECT_NAME_FOR_STRIPE_LISTENER=xxx" && \
  exit 1 \
);
...
ENV BWS_PROJECT_NAME_FOR_STRIPE_LISTENER=${BWS_PROJECT_NAME_FOR_STRIPE_LISTENER}
