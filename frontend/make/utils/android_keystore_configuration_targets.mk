...
# -----------------------------------------------------------------------------
# Makefile snippet to fetch Android Keystore from secrets (PROJECT_SECRETS_JSON),
...
/tmp/upload-keystore.p12:
	...
	@echo '$(PROJECT_SECRETS_JSON)' | jq -r '.KEYSTORE' | base64 --decode > /tmp/upload-keystore.p12
...
_export_android_keystore_vars:
	...
	$(eval export ANDROID_KEYSTORE_PASSWORD:=$(shell echo '$(PROJECT_SECRETS_JSON)' | jq -r '.KEYSTORE_PASS'))
	$(eval export ANDROID_KEY_ALIAS:=upload)
	$(eval export ANDROID_KEY_PASSWORD:=$(shell echo '$(PROJECT_SECRETS_JSON)' | jq -r '.KEYSTORE_PASS'))
	@echo "[INFO] [Export Android Keystore Vars] Android keystore environment variables exported."

_android_keystore_configuration: _bws_project_secrets_json /tmp/upload-keystore.p12 _export_android_keystore_vars
